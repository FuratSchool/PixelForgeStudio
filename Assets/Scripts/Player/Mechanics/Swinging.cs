...
public class Swinging : MonoBehaviour
{
    ...
    private void Update()
    {
        if (IsSwinging == false && InRange)
            CheckSwing();
        else if (_playerMovement.SwingPressed == false && IsSwinging)
            EndSwing();
        else
    ...
    private void CheckSwing()
    {
        if (_playerMovement._canSwing)
        {
            ...
            if (_playerMovement.SwingPressed)
            {
                ...
                lr.positionCount = 2;
                IsSwinging = true;
                _playerMovement._canSwing = false;
            }
        }
        else
        {
    ...
    private IEnumerator SwingDelayTimer()
    {
        ...
        _playerMovement._canSwing = true;
    }
